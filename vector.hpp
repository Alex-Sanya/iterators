...
template<typename T>
struct CustomVector {
    static_assert(is_same<typename remove_cv<T>::type, T>::value,
    ...
    void _realloc(size_type n) 
    {
        if (_start == nullptr)
        {
            _start = unique_ptr<value_type>(new T[n]);
            _finish = _start.get();
            _end_of_storage = _start.get() + n;
            return;
        }
        auto new_ptr = unique_ptr<value_type>(new T[n]);
        if (size() > 0)
            memcpy((void*)new_ptr.get(), (void*)_start.get(), min(n, size()) * sizeof(value_type));
        size_type prev_size = size();
        _start = move(new_ptr);
    ...
    void pop_back()
    {
        if (size() == 0)
        {
            throw out_of_range("Index cannot be equal or below zero");
        }
        _finish--;
    }
    ...
    reference at(size_type index) 
    {
        if (index >= size())
        {
            throw out_of_range("Index cannot be greater or equal of size of vector");
        }
        return (*this)[index];
    }
    ...
    value_type _move(const iterator& _begin, const iterator& _end, typename iterator::difference_type offset)
    {
        value_type tmp;
        iterator iter;
        for (iter = _begin; iter != _end; iter = next(iter, offset)) 
        {
